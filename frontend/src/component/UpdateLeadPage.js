...
const UpdateLeadPage = () => {
  ...
  useEffect(() => {
    const fetchLead = async () => {
      try {
        ...
        const response = await axios.get(`https://sai-lovat.vercel.app/api/leads/getSpecificLead/${id}`, {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`https://sai-lovat.vercel.app/api/leads/update/${id}`, { email, name, number, product }, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
